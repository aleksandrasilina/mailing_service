...
class SendEmailMessageTests(TestCase):
    def setUp(self):
        self.notification = Notification.objects.create(message="Test message", recipients=['test@example.com'],
                                                        delay=0)
    ...
    def test_send_email_message_success(self, mock_send_mail):
        ...
        send_email_message('Test message', self.notification.recipients, self.notification.pk)
...
class SendTelegramMessageTests(TestCase):
    ...
    def test_send_telegram_message_success(self, mock_get):
        ...
        send_telegram_message('Test message', self.notification.recipients, self.notification.pk)
        log = NotificationLog.objects.get(notification=self.notification.pk)
...
class SendNotificationsTaskTests(TestCase):
    ...
    def test_send_notifications(self, mock_send_telegram, mock_send_email):
        ...
        mock_send_email.assert_called_once_with(
            'Test message', ['test@example.com'], 1
        )
        mock_send_telegram.assert_called_once_with(
            'Test message', ['123456'], 1
        )
